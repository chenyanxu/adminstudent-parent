 ...
 * @version 1.0.0
 */
Ext.define('kalix.adminstudent.student.controller.UserGridController', {
  extend: 'kalix.controller.BaseGridController',
  alias: 'controller.studentUserGridController',
  requires: ['kalix.Notify'],
  onEdit: function (grid, rowIndex, colIndex) {
    var oldView = this.cfgForm;
    this.cfgForm = 'kalix.adminstudent.student.view.UserEditWindow';
  ...
  onKey: function (grid, rowIndex, colIndex) {
    ...
    var view = Ext.create('kalix.adminstudent.student.view.UserKeyWindow');
  ...
  onAuthorization: function (grid, rowIndex, colIndex) {
    /*var authorizationWindow = Ext.create('kalix.adminstudent.common.components.AuthorizationViewWindow');
    var rec = grid.getStore().getAt(rowIndex);
    authorizationWindow.roleId = rec.data.id;
    authorizationWindow.authorizationUrl = CONFIG.restRoot + '/camel/rest/students/' + rec.data.id + '/authorizations';
    ...
    store.setProxy({
      type: 'ajax',
      url: CONFIG.restRoot + '/camel/rest/students/' + rec.data.id + '/authorizations'
    });
